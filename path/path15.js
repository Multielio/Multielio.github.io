...
class Robot{
  ...
  getnear(p){
    var ppoint = createVector(p.path[0].x,p.path[0].y);
    var dis = p5.Vector.dist(ppoint,this.position);

    var ind = 0;
    for(var i =0; i<p.path.length;i++){
      ppoint = createVector(p.path[i].x,p.path[i].y);
      var distest = p5.Vector.dist(ppoint,this.position);
      if(distest<dis){
        ind = i;
        dis = distest;
      }
  ...
  lookahead(p,lookdistance){
    ...
    var goodi=0;
    while(i<(p.length-1)){
      var e = createVector(p[i].x,p[i].y);
      var l = createVector(p[i+1].x,p[i+1].y);
    ...
    var ee = createVector(p[goodi].x,p[goodi].y);
    var ll = createVector(p[goodi+1].x,p[goodi+1].y);
...
function setup() {
  ...
  corner1 = createVector(200,200);
  corner2 = createVector(350,350);
...
function draw() {
  if(a ==1){
    ...
    m = new MapRRTStar(createVector(xinit,yinit),corner1,corner2,cible);
    pp = m.gen(50,200);
