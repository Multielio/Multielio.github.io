...
class Box{
  ...
  bdraw(d){
    noStroke();
    rectMode(CENTER);
    fill(this.z*2.5,random(50)*0.1*(height-this.y)*sin(exp(this.teta)),d*random(150),this.z*this.teta*2.5);
    ellipse(this.x,this.y,30,10,2);
    this.teta +=this.direc*0.001;
    ...
    if(this.teta>0.5 || this.teta<0){

      this.direc =-1*this.direc;
    }
  }
